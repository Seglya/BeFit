...
namespace BeFit.Repositories
{
    ...
    public class FillMeasurementRepository : IFillMeasurementRepository
    {
        ...
        {
            if (Id != 0)
            {   var ret= await _context.FillMeasurement.AsNoTracking().SingleOrDefaultAsync(i => i.FillMeasurementID == Id);
            return ret;}
            return null;
        }
        ...
        {
            if (userId != 0)
                return _context.FillMeasurement.Include(m=>m.Measurement).Where(s => s.AppUserID == userId & s.Date==date);
            return null;
        }

        public async Task<FillMeasurement> CreateOrEditFillMeasurementAsync(int id,AddMeasurementViewModel viewModel )
        {
            FillMeasurement fill = new FillMeasurement ();
            if (id != 0)
            {
        ...
        {List<FillMeasurement> delList=new List<FillMeasurement>();
